...
function Dashboard({ user, onLogout, onUpdateUser }) {
    ...
    const revenueBreakdown = React.useMemo(() => {
        ...
        bookings.forEach(order => {
            ...
            if (date >= startOfThisMonth) monthly += amt;
            if (date >= startOfThisYear) yearly += amt;
        });
    ...
    const currentAlerts = React.useMemo(() => {
        ...
        // Calculate the most ordered item for the current user
        ...
        if (userOrders && userOrders.length > 0) {
            ...
            userOrders.forEach(order => {
                ...
                itemsList.forEach(item => {
                    if (item && item.food_name) {
                        itemCounts[item.food_name] = (itemCounts[item.food_name] || 0) + (item.quantity || 1);
                    }
                });
            });
        ...
        if (menuStock && menuStock.length > 0) {
            ...
            if (favoriteItem) {
                dynamic.push(`🌟 ${favoriteItem} is your most ordered item!`);
            } else {
                ...
                {view === "dashboard" ? (
                    isAdmin ? (
                            ...
                            {adminView === "analytics" ? (
                                ...
                                <section className="analytics-section">
                                    ...
                                    <div className="analytics-stats-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                                        ...
                                        <div className="analytics-stat-card stat-revenue" style={{ background: "linear-gradient(135deg, white, #fdf4ff)", cursor: 'pointer', position: 'relative' }} onClick={() => setShowRevenueBreakdown(!showRevenueBreakdown)}>
                                            ...
                                            <div className="stat-card-info" style={{ width: '100%' }}>
                                                <p className="stat-card-label">Total Revenue <span style={{ fontSize: '0.75rem', color: '#9ca3af' }}>(Click to expand)</span></p>
                                                <p className="stat-card-value" style={{ marginBottom: showRevenueBreakdown ? '10px' : '0' }}>
                                                    <AnimatedCounter target={analyticsStats.totalRevenue || 0} prefix="₹" />
                                                </p>
                                                {showRevenueBreakdown && (
                                                    <div style={{ backgroundColor: '#f3e8ff', padding: '10px', borderRadius: '8px', fontSize: '0.9rem', color: '#6b21a8' }}>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span>Weekly:</span> <strong>₹{revenueBreakdown.weekly}</strong></div>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span>Monthly:</span> <strong>₹{revenueBreakdown.monthly}</strong></div>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Yearly:</span> <strong>₹{revenueBreakdown.yearly}</strong></div>
                                                    </div>
                                                )}
                                            </div>
                        ...
                        <>
                            ...
                            {/* Events Display for Students and Faculty */}
                            <section className="student-events-section" style={{ marginTop: '2rem', padding: '1.5rem', background: 'white', borderRadius: '16px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
                                <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '1rem', color: '#1e293b' }}>Upcoming Events</h3>
                                {events && events.length > 0 ? (
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
                                        {events.map(event => (
                                            <div key={event.id} style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px', background: '#f8fafc' }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                                    <h4 style={{ margin: 0, color: '#0f172a', fontSize: '16px' }}>{event.title}</h4>
                                                    <span style={{ fontSize: '12px', color: '#6366f1', background: '#e0e7ff', padding: '2px 8px', borderRadius: '12px', fontWeight: '600', textTransform: 'capitalize' }}>{event.type || 'Event'}</span>
                                                </div>
                                                <p style={{ margin: '0 0 12px 0', fontSize: '14px', color: '#475569' }}>{event.description}</p>
